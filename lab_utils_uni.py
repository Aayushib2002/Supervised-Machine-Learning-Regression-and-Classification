...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    ...
    # History arrays
    # =========================
    x = np.zeros(len(p_hist), dtype=np.float64)
    y = np.zeros(len(p_hist), dtype=np.float64)
    v = np.zeros(len(p_hist), dtype=np.float64)
    ...
    w_array = np.linspace(-1000, 1000, 200, dtype=np.float64)
    cost = np.zeros_like(w_array, dtype=np.float64)

    for i in range(len(w_array)):
        tmp_w = w_array[i]
        c = compute_cost(x_train, y_train, tmp_w, fix_b)
    ...
    # =========================
    # Second subplot: Cost surface
    # =========================
    w_vals = np.linspace(-2000, 2000, 80, dtype=np.float64)
    b_vals = np.linspace(-1000, 1000, 80, dtype=np.float64)

    tmp_b, tmp_w = np.meshgrid(b_vals, w_vals)
    z = np.zeros_like(tmp_w, dtype=np.float64)

    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            c = compute_cost(x_train, y_train, tmp_w[i, j], tmp_b[i, j])
